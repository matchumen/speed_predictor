...
def define_model(params):
    ...
    for i in range(params['N_LAYERS'] - 1):
        layers.append((f'fc{i}', nn.Linear(input_shape, input_shape // 2)))
        layers.append((f'ac{i}', getattr(nn, params['ACTIVATION_NAME'])()))
        input_shape = input_shape // 2

    layers.append((f'fcout', nn.Linear(input_shape, 1)))
...
def objective(trial):
    FW_SAMPLES = trial.suggest_categorical('FW_SAMPLES', [10])
    BW_SAMPLES = trial.suggest_categorical('BW_SAMPLES', [10])

    #prepare data for dataloader
    train_data = []
    for df in train_dfs:
        train_data += utils.make_dataloader_data(df, window_size=((BW_SAMPLES, FW_SAMPLES)))

    val_data = []
    for df in val_dfs:
        val_data += utils.make_dataloader_data(df, window_size=(BW_SAMPLES, FW_SAMPLES))
...
if __name__ == '__main__':
    ...
    train_dfs = utils.load_preprocess_drives(train_drives, drop_null_cols=True, blur_loc_features=True, add_stop_feature=True)
    val_dfs = utils.load_preprocess_drives(val_drives, drop_null_cols=True, blur_loc_features=True, add_stop_feature=True)
